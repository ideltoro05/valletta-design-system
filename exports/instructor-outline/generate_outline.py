...
def clean(t):
    # fix source line-wrap hyphenation artifacts ("word- word" -> "wordword") and a mis-encoded apostrophe
...
def header():
    return f'''<div class="doc-header">
      ...
      <div class="doc-tag">Leadership Instructor Outline</div>
    </div>'''

def footer():
    return '<div class="doc-footer">Valletta Industries &nbsp;|&nbsp; Leadership Instructor Outline<span class="pagenum"></span></div>'

def sheet(inner, klass=""):
    return f'<section class="sheet {klass}">{inner}</section>'

def title(text):
    return f'<div class="h1-title">{clean(text)}</div>'

def subhead(text):
    return f'<div class="h2-sub">{clean(text)}</div>'

def p(text):
    return f'<p class="body-p">{clean(text)}</p>'

def note(tag, lines):
    body = "".join(f'<div class="note-line">{clean(l)}</div>' for l in lines)
    return f'<div class="note-box"><span class="note-tag">{tag}</span>{body}</div>'

def bio(text):
    return f'<p class="bio-aside">{clean(text)}</p>'

def quote(text, attribution=None, lead=None):
    lead_html = f'<span class="q-lead">{clean(lead)}</span> ' if lead else ""
    attr_html = f' &ndash; <span class="q-attr">{clean(attribution)}</span>' if attribution else ""
    return f'<div class="quote">{lead_html}<span class="q-mark">&ldquo;</span>{clean(text)}<span class="q-mark">&rdquo;</span>{attr_html}</div>'

def stat(text):
    return f'<li>{clean(text)}</li>'

def meta_row(label, value):
    return f'<div class="meta-row"><span class="meta-label">{label}</span><span class="meta-value">{clean(value)}</span></div>'
...
COVER_HTML = sheet(f'''
  ...
  <div class="cover-eyebrow">Instructor Outline</div>
...
presentation = (
    ...
    + note("SCRIPT", ["Good morning/afternoon/evening, my name is ____. Your next period of instruction will be on "
                       "becoming an effective leader. (Instructor Experience)"])
    + subhead("Objective")
    + p("The objective of this course is to learn the fundamentals of being a leader. As a supervisor, manager, or "
        "task leader you will be around people from different backgrounds and with different past experiences, and "
        "each may require a different style of leadership. It is your responsibility to effectively lead and "
        "complete assigned tasks while dealing with different personalities. Your people (subordinates) will "
        "determine if you are an effective or ineffective leader, and this can be measured by performance of "
        "assigned tasks. Effective leadership begins with having the proper mindset and attitude.")
)
BLOCKS.append(("presentation", presentation))

video1 = note("VIDEO 1 — SGT. TRAVIS MILLS", [
...
overview = (
    ...
    + "".join(f'<li>{t}</li>' for t in [
        "Defining Leadership", "Examples of Effective Leadership",
        "Describe Components of Being an Effective Leader", "Ethical Characteristics for Leadership",
        "Describe Motivational Leadership", "Understand the Basics of Supervision and Evaluating Work Performance",
    ]) + '</ul>'
)
...
defining = (
    ...
    + p("Definition—The ability to influence others to accomplish a task by providing a purpose, a direction, "
        "and appropriate motivation. Explaining the mission to subordinates helps them focus on the importance of "
        "the objective and allows them to perform in a more efficient and disciplined manner.")
    + p("Although having a supervisory position provides the opportunity to be a leader; a position does not make "
        "you a leader. Being a leader is a choice that has nothing to do with your status on the organizational "
        "chart; once you decide to lead others, it will become easier to see where you can impact the mission the "
        "most.")
    + p("We have all heard the phrase “Leaders Lead,” but not all leaders are influential. Leaders must "
        "set an example for all to follow. Good leaders set the standard and can meet it/surpass it. Leading by "
        "example instills confidence and respect from subordinates and makes the leader more effective.")
    + p("Strive to be the best at your assignments and learn the positions above and below you. Understanding the "
        "tasks assigned to personnel on your team will build confidence in your leadership.")
    + p("A leader looks for ways to better those around them and the mission. A leader looks for more "
        "responsibility and accepts the outcome no matter the success of your actions.")
    + p("Make sound and timely decisions. A good leader makes a decision based on the immediately available "
        "information. As a leader being indecisive can result in problems getting worse rather than better. An "
        "immediate decision is still moving the team closer to completing the mission. If a decision does not get "
        "the desired result, discuss with subordinates and other leaders to learn from it.")
)
BLOCKS.append(("defining", defining))

# ---- Examples of Effective Leadership ----
examples = (
    title("Examples of Effective Leadership")
    + p("Charismatic Leader—Charismatic leadership combines charm, interpersonal connection, and persuasive "
        "communication to motivate others. Some examples are Sir Winston Churchill, and Nelson Mandela.")
    + p("Transformational Leader—Transformational leadership is when a leader’s behaviors influence "
        "followers and inspire them to perform beyond their perceived capabilities. Transformational leadership "
        "inspires people to achieve unexpected or remarkable results. Examples are Dr. Martin Luther King, Henry "
        "Ford, and Steve Jobs")
    + p("Servant Leader—Servant leadership occurs when the leader’s main goal and responsibility is to "
        "provide service to their people. Examples are Mahatma Gandhi and Mother Teresa.")
    + p("Quiet Leaders—Quiet leaders recognize they cannot ask someone to do a task they wouldn’t or "
        "haven’t done. They set the example for others to follow. Silent leaders hold themselves to the same "
        "(or higher) standard than they hold for others. Trust. Silent leaders do not micromanage or nag others. "
        "Examples are Albert Einstein, Sir Isaac Newton, Rosa Parks, Bill Gates, and Warren Buffett.")
...
BLOCKS.append(("examples", examples))

# ---- Components: Leadership Traits ----
traits = (
    title("Describe the Components of Effective Leadership")
    + subhead("Leadership Traits")
    + p("A good leader encourages their staff to participate in all assigned tasks with clear instructions and "
        "supervision.")
    + p("A good leader educates themselves to understand assigned tasks and mentor those below them. Learning the "
        "functions of their supervisors provide context for the direction of the mission and makes the leader a "
        "more valuable member of the team.")
    + p("A good leader knows their capabilities and has an honest opinion of their strengths and weaknesses. This "
        "information is discovered through honest evaluation by supervisors and subordinates during regular "
        "performance discussions.")
    + p("A good leader treats all team members with dignity and respect. Respect in its basic form shows that you "
        "are aware of someone’s rights, values, wishes, and other personal beliefs. To be aware of this "
        "information, a leader must regularly communicate to understand each team member’s individual needs. "
        "A leader always must respect subordinates to earn their respect.")
    + p("A leader must possess professional character traits such as ambition, creativity, compassion, courage, "
        "flexibility, honesty (integrity), humility, loyalty, patience, discipline, and curiosity.")
    + p("A leader understands how to manage stress and is capable of sharing those skills. Exercise, meditation, "
        "breathing exercises, and reading are all examples of stress-relieving activities. Also, honest "
        "discussions about specific topics can help relieve stress by providing clarity about the stressful issue")
)
BLOCKS.append(("traits", traits))

# ---- Communication ----
communication = (
    subhead("Communication")
    + p("Influential leaders must adapt their communication style to effectively communicate to their team.")
    + p("Each employee will have different motivations, so understanding how to communicate with each team member "
        "is essential.")
    + p("Effective leaders must be able to talk but also listen. It is essential to listen to your team and "
        "genuinely accept their input even if their suggestions are not part of the leader’s final "
        "instructions.")
    + p("An effective leader must be transparent about the team’s direction to continue building trust with "
        "subordinates. Honestly assessing performance and acknowledging if the leader has made a mistake will "
        "encourage employees that they are safe to share information with the leader.")
    + p("Effective leaders use communication tools to understand subordinate’s suggestions better. By asking "
        "open-ended questions like “tell me more” or “explain what you mean,” a leader can get "
        "more details and ensure they understand the information thoroughly.")
    + p("Communication is not only verbal but how you carry yourself. Practicing simple body language cues will "
        "help establish interest and rapport. Making eye contact is a simple way to start changing body language "
        "cues.")
    + p("A good leader will ask for feedback on their performance to subordinates, and understanding what "
        "subordinates need from the leader. The most important part of this is to implement change based on their "
        "input. The effort to implement change will continue to strengthen the trust of subordinates.")
)
BLOCKS.append(("communication", communication))

# ---- Observing Effective Follower Traits ----
followers_a = (
    title("Observing Effective Follower Traits (Knowing Your People)")
    + quote(
...
BLOCKS.append(("followers_a", followers_a))
followers_b = (
    p("Followers are an essential part of leadership, and being a follower is a straightforward concept. "
        "Following is the ability to take direction well, believe in a program, be part of a team, and deliver on "
        "your expectations. A follower can provide strength to the leader by contributing to the organization in "
        "areas that complement the leader’s position.")
    + p("A follower has good judgment and will take direction but not follow blindly. Good judgment is required "
        "and can be learned from good leadership.")
    + p("Good followers are good workers, and they need good direction to understand what is expected of them.")
    + p("Followers can be categorized into multiple groups, such as followers with positive attitudes who will be "
        "motivated on their own. Others will have minimal levels of independent thinking and need constant "
        "direction. Also, negative followers will be more capable of thinking independently but do not contribute "
        "positively to the team. We can break these groups down further, but for this course, it is only "
        "essential to understand that each follower will be different in the leadership style that will inspire "
        "them the most.")
    + p("To be effective, followers must be willing to share their ideas and opinions with leaders.")
)
BLOCKS.append(("followers_b", followers_b))

# ---- Understanding the Mission or Task ----
mission_a = (
    title("Understanding the Mission or Task")
    + p("As a leader, you must have a purpose, and your drive will come from your mission.")
    + p("The mission will help the team share priorities, set performance goals, plan performance rewards, and "
        "give a shared vision for the team.")
    + p("The mission or task also gives an effective leader “vision.” Vision is the opportunity to see a "
        "path to success or failure and the future development of your team.")
    + p("An effective leader can create small tasks to create growth on their team. A leader can plan short "
        "team-building exercises or task-specific training to focus the team on an assigned task. Often an "
        "effective will be managing multiple tasks for each member on their team based on the team member’s "
        "ability.")
    + note("VIDEO 4", ["The first 3:27 of – https://www.youtube.com/watch?v=jYK-drFNI6o",
                        "Jocko Willink/Mulligan Brothers interview that describes timing and response"])
    + note("DISCUSSION", ["Focusing on the appropriate time to execute decisions.",
                           "Picking the right time to action change, (leadership capital)."])
)
BLOCKS.append(("mission_a", mission_a))
mission_b = (
    bio(
...
BLOCKS.append(("mission_b", mission_b))

# ---- Ethical Characteristics ----
ethical = (
    title("Ethical Characteristics for Leadership")
    + p("<strong>Fair</strong>—An ethical leader is fair, and they treat everyone on their team equally with "
        "no bias. Team members will not perform to their best ability if they feel like decisions are unfair and "
        "made with any bias.")
    + p("<strong>Honest</strong>—Ethical leaders are honest and must always be transparent and fair. A good "
        "leader should give the unpopular truth to their team rather than a lie. Consistent honesty builds trust "
        "with your team and promotes the open sharing of information.")
    + p("<strong>Respect</strong>—Leaders who actively listen to subordinates and do not dismiss their "
        "concerns will have a team focused on achieving the same objective. Although you may be higher in a chain "
        "of command, your team should feel as though they are equally important to the mission.")
    + p("<strong>Value-Oriented</strong>—While each person has individual values, a good leader will make "
        "decisions based on the organization’s values. An ethical leader will share the organization’s "
        "values with the team and implement changes that adhere to those values.")
    + p("<strong>Leads by Example</strong>—Employees will mimic the action of their leader. A leader cannot "
        "expect his team to make ethical decisions if they are unwilling to make ethical choices. An ethical "
        "leader takes ownership of their actions without placing blame on anyone else.")
    + p("<strong>Prioritizes the Team</strong>—Ethical leaders will promote team building and foster a sense "
        "of community. Ethical leaders will help each team member to achieve personal and professional goals "
...
BLOCKS.append(("ethical", ethical))

# ---- Motivational Leadership ----
motivational_a = (
    title("Describe Motivational Leadership")
    + quote(
...
BLOCKS.append(("motivational_a", motivational_a))
motivational_b = (
...
BLOCKS.append(("motivational_b", motivational_b))

# ---- Supervision ----
supervision = (
    title("Understand the Basics of Supervision and Evaluating Work Performance")
...
BLOCKS.append(("supervision", supervision))
...
with open(os.path.join(HERE, "valletta_instructor_outline.html"), "w") as f:
    f.write(html)
