...
function outlineItem(label, text, level = 1, bold = false, opts = {}) {
  const runs = [];
  if (label) runs.push(new TextRun({ text: label + " ", font: F_BODY, bold: true, size: 21, color: RED }));
  if (typeof text === "string") {
    runs.push(new TextRun({ text, font: F_BODY, size: 21, bold, color: bold ? BLACK : CHARCOAL }));
  } else {
    runs.push(...text);
  }
  return new Paragraph({
    indent: { left: IND[level] },
    spacing: { after: opts.after ?? 90, line: 260 },
    children: runs,
  });
}
...
function secTitle(num, title) {
  return new Paragraph({
    ...
    children: [new TextRun({ text: `${num}.  ${title.toUpperCase()}`, font: F_HEAD, bold: true, size: 27, color: BLACK })],
  });
}
...
function overviewBlock(items) {
  ...
  const rows = items.map((t, i) => new Paragraph({
    ...
    children: [
      new TextRun({ text: `${i + 1}. `, font: F_BODY, bold: true, size: 21, color: RED }),
...
content.push(
  secTitle(1, "Defining Leadership"),
  ...
  secTitle(2, "Examples of Effective Leadership"),
  outlineItem("A.", "Charismatic Leader", 1),
  outlineItem("B.", "Transformational Leader", 1),
  outlineItem("C.", "Servant Leader", 1),
  outlineItem("D.", "Quiet Leaders", 1),

  secTitle(3, "Describe the Components of Effective Leadership"),
  outlineItem("A.", "Leadership Traits", 1, true),
...
content.push(
  outlineItem("B.", "Communication", 1, true),
  ...
  outlineItem("C.", "Observing Effective Follower Traits (Knowing your People)", 1, true),
  quoteBlock(
    ...
    "our human history, most humans were in small, nomadic clusters. These tribes offered protection, food, and " +
  ...
  outlineItem("D.", "Understanding the Mission or Task", 1, true),
  ...
  secTitle(4, "Ethical Characteristics for Leadership"),
  outlineItem("A.", [new TextRun({ text: "Fair", font: F_BODY, bold: true, size: 21, color: BLACK }), new TextRun({ text: " — An ethical leader is fair, and they treat everyone on their team equally with no bias.", font: F_BODY, size: 21, color: CHARCOAL })], 1),
  outlineItem("B.", [new TextRun({ text: "Honest", font: F_BODY, bold: true, size: 21, color: BLACK }), new TextRun({ text: " — Ethical leaders are honest and must always be transparent and fair.", font: F_BODY, size: 21, color: CHARCOAL })], 1),
  outlineItem("C.", [new TextRun({ text: "Respect", font: F_BODY, bold: true, size: 21, color: BLACK }), new TextRun({ text: " — Listening to subordinate and valuing opinions of the team.", font: F_BODY, size: 21, color: CHARCOAL })], 1),
  outlineItem("D.", [new TextRun({ text: "Value-Oriented", font: F_BODY, bold: true, size: 21, color: BLACK }), new TextRun({ text: " — A good leader will make decisions based on the organization’s values, and the values of the mission.", font: F_BODY, size: 21, color: CHARCOAL })], 1),
  outlineItem("E.", [new TextRun({ text: "Leads by Example", font: F_BODY, bold: true, size: 21, color: BLACK }), new TextRun({ text: " — Employees will mimic the action of their leader.", font: F_BODY, size: 21, color: CHARCOAL })], 1),
  outlineItem("F.", [new TextRun({ text: "Prioritizes the Team", font: F_BODY, bold: true, size: 21, color: BLACK }), new TextRun({ text: " — Ethical leaders will promote team building and foster a sense of community.", font: F_BODY, size: 21, color: CHARCOAL })], 1),
  quoteBlock(
...
content.push(
  secTitle(5, "Describe Motivational Leadership"),
  quoteBlock("The only thing more contagious than a good attitude is a bad one.", "David Goggins"),
  outlineItem("A.", "A motivational leader can make decisions and set clear goals for their teams.", 1),
  outlineItem("B.", "What is the difference between Motivation and Inspiration?", 1),
  outlineItem("1.", "Inspiration is an external influence or pulling force that compels you to achieve something.", 2),
  outlineItem("2.", "Motivation is an internal influence or a driving force that you feel on the inside to achieve something.", 2),
  outlineItem("C.", "A good leader should try to inspire those around them.", 1),
  outlineItem("D.", "An effective leader will make logical decisions based on the immediate information available. If the decision was wrong, explain the rationale behind it or what led you to the conclusion and learn from it.", 1),
  outlineItem("–", [new TextRun({ text: "OODA Loop:", font: F_BODY, bold: true, size: 21, color: BLACK }), new TextRun({ text: " Observe, Orient, Decide, and Act", font: F_BODY, size: 21, color: CHARCOAL })], 2),
  outlineItem("1.", "Know the strengths and weaknesses of subordinates and look out for their wellbeing.", 2),
  quoteBlock(
  ...
  secTitle(6, "Understand the Basics of Supervision and Evaluating Work Performance"),
  outlineItem("A.", "An effective leader does not Micro-Manage.", 1),
  outlineItem("B.", "An effective leader is proactive and should avoid having a “hands-off” mentality.", 1),
  outlineItem("C.", "A leader will delegate supervision, responsibility, and authority to capable subordinates.", 1),
  outlineItem("D.", "A leader should ensure that all tasks are understood, supervised, and accomplished.", 1),
  outlineItem("E.", "A good supervisor can perform the tasks of his subordinates", 1),
  outlineItem("F.", "A good leader will hold their team accountable with honest feedback on performance from each assigned task.", 1),
...
Packer.toBuffer(doc).then((buf) => {
  fs.writeFileSync(path.join(HERE, "valletta_leadership_handout.docx"), buf);
