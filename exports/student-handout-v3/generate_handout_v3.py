...
def bullet(text):
    return f'<li class="plain-bullet">{text}</li>'

def quote(text, attribution=None, lead=""):
...
def sec_title(letter, title):
    return f'<div class="sec-title"><span class="sec-num">{letter}.</span> {title}</div>'
...
def overview():
    ...
    lis = "".join(f'<li><span class="ov-num">{l}.</span> {t}</li>' for l, t in items)
...
sec_a = sec_title("A", "Defining Leadership") + "".join([
    item("a.", "Definition &ndash; The ability to influence others to accomplish a task by providing a purpose, a direction, and appropriate motivation.", 1),
...
sec_b = sec_title("B", "Examples of Effective Leadership") + '<ul class="plain-list">' + "".join([
    bullet("Charismatic Leader"),
    bullet("Transformational Leader"),
    bullet("Servant Leader"),
    bullet("Quiet Leaders"),
]) + '</ul>'

BLOCKS.append(("overview", overview()))
BLOCKS.append(("sec_a", sec_a))
BLOCKS.append(("sec_b", sec_b))

sec_c_traits = sec_title("C", "Describe the Components of Effective Leadership") + item("", "Leadership Traits", 1, bold=True) + "".join([
...
sec_c_comm = item("", "Communication", 1, bold=True) + "".join([
...
sec_c_followers_head = item("", "Observing Effective Follower Traits (Knowing your People)", 1, bold=True)
...
sec_c_mission = item("", "Understanding the Mission or Task", 1, bold=True) + "".join([
...
BLOCKS.append(("sec_c_followers", sec_c_followers_head + followers_quote + sec_c_followers_rest))
BLOCKS.append(("sec_c_mission", sec_c_mission))

sec_d = sec_title("D", "Ethical Characteristics for Leadership") + "".join([
    item("", "<strong>Fair</strong> &ndash; An ethical leader is fair, and they treat everyone on their team equally with no bias.", 1),
    item("", "<strong>Honest</strong> &ndash; Ethical leaders are honest and must always be transparent and fair.", 1),
    item("", "<strong>Respect</strong> &ndash; Listening to subordinate and valuing opinions of the team.", 1),
    item("", "<strong>Value-Oriented</strong> &ndash; A good leader will make decisions based on the organization's values, and the values of the mission.", 1),
    item("", "<strong>Leads by Example</strong> &ndash; Employees will mimic the action of their leader.", 1),
    item("", "<strong>Makes the team a priority</strong> &ndash; Ethical leaders will promote team building and foster a sense of community.", 1),
]) + quote(
...
sec_e = sec_title("E", "Describe What Motivational Leadership Is") + quote(
    "The only thing more contagious than a good attitude is a bad one.", "David Goggins.") + "".join([
    item("", "A motivational leader can make decisions and set clear goals for their teams.", 1),
    item("", "What is the difference between Motivation and Inspiration?", 1),
    item("", "Inspiration is an external influence or pulling force that compels you to achieve something.", 2),
    item("", "Motivation is an internal influence or a driving force that you feel on the inside to achieve something.", 2),
    item("", "A good leader should try to inspire those around them.", 1),
    item("", "An effective leader will make logical decisions based on the immediate information available. If the decision was wrong, explain the rationale behind it or what led you to the conclusion and learn from it.", 1),
    item("&ndash;", "<strong>OODA loop:</strong> Observe, Orient, Decide, and Act.", 2),
    item("", "Know the strengths and weaknesses of subordinates and look out for their wellbeing.", 1),
]) + quote(
...
sec_f = sec_title("F", "Understand the Basics of Supervision and Evaluating Work Performance") + "".join([
    item("", "An effective leader does not Micro-Manage.", 1),
    item("", "An effective leader is proactive and should avoid having a &ldquo;hands-off&rdquo; mentality.", 1),
    item("", "A leader will delegate supervision, responsibility, and authority to capable subordinates.", 1),
    item("", "A leader should ensure that all tasks are understood, supervised, and accomplished.", 1),
    item("", "A good supervisor can perform the tasks of his subordinates", 1),
    item("", "A good leader will hold their team accountable with honest feedback on performance from each assigned task.", 1),
])
...
.plain-list {{ list-style:none; padding-left:4px; margin-bottom:6px; }}
.plain-bullet {{ position:relative; padding-left:15px; font-size:11px; line-height:1.6; margin-bottom:5px; color:{CHARCOAL}; }}
.plain-bullet::before {{ content:''; position:absolute; left:0; top:5px; width:6px; height:6px; background:{RED}; }}
...
with open(os.path.join(HERE, "valletta_student_handout_v3.html"), "w") as f:
    f.write(html)
