...
function secHeading(letter, text) {
  return new Paragraph({
    spacing: { before: 260, after: 140 },
    border: { bottom: { color: RED, space: 4, style: BorderStyle.SINGLE, size: 10 } },
    children: [
      new TextRun({ text: letter + ".  ", font: F_HEAD, bold: true, size: 27, color: RED }),
      new TextRun({ text: text.toUpperCase(), font: F_HEAD, bold: true, size: 27, color: BLACK }),
    ],
  });
}
...
function bodyPara(text) {
  return new Paragraph({ spacing: { after: 130, line: 270 }, children: parseInline(text) });
}
...
function leadIn(label, text) {
  return new Paragraph({
    ...
    children: [
      new TextRun({ text: label + " ", font: F_BODY, bold: true, size: 21, color: BLACK }),
      new TextRun({ text, font: F_BODY, size: 21, color: CHARCOAL }),
    ],
  });
}
...
function outlineItem(label, text, level = 0) {
  return new Paragraph({
    ...
    children: [
      new TextRun({ text: label + " ", font: F_BODY, bold: true, size: 21, color: RED }),
      new TextRun({ text, font: F_BODY, size: 21, color: CHARCOAL }),
    ],
  });
}
...
function conclusionBand(title, text) {
  const banner = new Paragraph({
    shading: { type: ShadingType.CLEAR, color: "auto", fill: BLACK },
    spacing: { after: 100 },
    children: [new TextRun({ text: title.toUpperCase(), font: F_HEAD, bold: true, size: 24, color: WHITE })],
  });
  ...
  return new Header({
    children: [new Paragraph({
      ...
      children: [
        ...
        new TextRun({ text: "\t" }),
        new TextRun({ text: "LEADERSHIP TRAINING OUTLINE", font: F_LABEL, bold: true, size: 15, color: STEEL, characterSpacing: 10 }),
      ],
    })],
  });
}
function makeFooter() {
  return new Footer({
    children: [new Paragraph({
      ...
      children: [
        new TextRun({ text: "Valletta Industries  |  Leadership Training Outline  –  Page ", font: F_BODY, size: 15, color: STEEL }),
...
content.push(
  new Paragraph({
    border: { top: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, bottom: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, left: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, right: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 } },
    tabStops: [{ type: "left", position: 2600 }],
    spacing: { before: 60, after: 60 },
    children: [new TextRun({ text: "TIME\t", font: F_LABEL, bold: true, size: 15, color: RED, characterSpacing: 4 }), new TextRun({ text: "4 hours", font: F_BODY, size: 20, color: CHARCOAL })],
  }),
  new Paragraph({
    border: { bottom: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, left: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, right: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 } },
    tabStops: [{ type: "left", position: 2600 }],
    spacing: { after: 60 },
    children: [new TextRun({ text: "METHOD OF INSTRUCTION\t", font: F_LABEL, bold: true, size: 15, color: RED, characterSpacing: 4 }), new TextRun({ text: "Classroom lecture, PowerPoint presentation, classroom discussion, and observation/review during practical exercises.", font: F_BODY, size: 20, color: CHARCOAL })],
  }),
  new Paragraph({
    border: { bottom: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, left: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, right: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 } },
    tabStops: [{ type: "left", position: 2600 }],
    spacing: { after: 60 },
    children: [new TextRun({ text: "DESCRIPTION\t", font: F_LABEL, bold: true, size: 15, color: RED, characterSpacing: 4 }), new TextRun({ text: "In this period of instruction, we will discuss the importance of leadership principles and the components it takes to be an effective leader.", font: F_BODY, size: 20, color: CHARCOAL })],
  }),
  new Paragraph({
    border: { bottom: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, left: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, right: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 } },
    tabStops: [{ type: "left", position: 2600 }],
    spacing: { after: 60 },
    children: [new TextRun({ text: "SPECIAL REQUIREMENTS\t", font: F_LABEL, bold: true, size: 15, color: RED, characterSpacing: 4 }), new TextRun({ text: "Classroom, laptop computer, presentation pointer, projector, and PowerPoint presentation. (Can be conducted by lecture only from the outline)", font: F_BODY, size: 20, color: CHARCOAL })],
  }),
  new Paragraph({
    border: { bottom: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, left: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 }, right: { color: LINE_GRAY, space: 8, style: BorderStyle.SINGLE, size: 4 } },
    tabStops: [{ type: "left", position: 2600 }],
    spacing: { after: 200 },
    children: [new TextRun({ text: "INSTRUCTIONAL AIDES\t", font: F_LABEL, bold: true, size: 15, color: RED, characterSpacing: 4 }), new TextRun({ text: "PowerPoint presentation, student outlines, handouts", font: F_BODY, size: 20, color: CHARCOAL })],
  }),
);

// ---- Presentation ----
content.push(
  plainTitle("Presentation"),
...
content.push(
  secHeading("A", "Defining Leadership"),
  ...
  outlineItem("a.", "Definition — The ability to influence others to accomplish a task by providing a purpose, a direction, and appropriate motivation. Explaining the mission to subordinates helps them focus on the importance of the objective and allows them to perform in a more efficient and disciplined manner."),
  outlineItem("b.", "Although having a supervisory position provides the opportunity to be a leader; a position does not make you a leader. Being a leader is a choice that has nothing to do with your status on the organizational chart; once you decide to lead others, it will become easier to see where you can impact the mission the most."),
  outlineItem("c.", "We have all heard the phrase “Leaders Lead,” but not all leaders are influential. Leaders must set an example for all to follow. Good leaders set the standard and can meet it/surpass it. Leading by example instills confidence and respect from subordinates and makes the leader more effective."),
  outlineItem("d.", "Strive to be the best at your assignments and learn the positions above and below you. Understanding the tasks assigned to personnel on your team will build confidence in your leadership."),
  outlineItem("e.", "A leader looks for ways to better those around them and the mission. A leader looks for more responsibility and accepts the outcome no matter the success of your actions."),
  outlineItem("f.", "Make sound and timely decisions. A good leader makes a decision based on the immediately available information. As a leader being indecisive can result in problems getting worse rather than better. An immediate decision is still moving the team closer to completing the mission. If a decision does not get the desired result, discuss with subordinates and other leaders to learn from it."),

  secHeading("B", "Examples of Effective Leadership"),
  bodyPara("Charismatic Leader — Charismatic leadership combines charm, interpersonal connection, and persuasive communication to motivate others. Some examples are Sir Winston Churchill, Malcolm X, and Nelson Mandela."),
...
// ---- C. Components of effective leadership ----
content.push(
  secHeading("C", "Describe the Components of Effective Leadership"),
...
content.push(
  subHeading("Observing Effective Follower Traits (Knowing your People)"),
  ...
  subHeading("Understanding the Mission or Task"),
...
content.push(
  ...
  secHeading("D", "Ethical Characteristics for Leadership"),
  ...
  bodyPara("<strong>Makes the team a priority</strong> — Ethical leaders will promote team building and foster a sense of community. Ethical leaders will help each team member to achieve personal and professional goals through regular communication."),
...
// ---- E. Motivational Leadership ----
content.push(
  secHeading("E", "Describe What Motivational Leadership Is"),
...
// ---- F. Supervision / Conclusion / Additional Reading ----
content.push(
  secHeading("F", "Understand the Basics of Supervision and Evaluating Work Performance"),
...
Packer.toBuffer(doc).then((buf) => {
  fs.writeFileSync(path.join(HERE, "valletta_leadership_outline_v3.docx"), buf);
